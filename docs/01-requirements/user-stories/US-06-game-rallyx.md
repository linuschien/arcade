...
- **AC1 (Inertial Cruise & Never Stops)**：藍車在迷宮通道中保持定速自動向前行駛，無手動煞車減速機制，速度永遠大於 0，遊戲進行期間藍車絕不靜止停滯。
...
- **AC4 (Clockwise Auto-Turn on Corners & T-Junctions)**：當藍車行駛至 L 型直角彎或迎面遇 T 字路口撞牆時：
  - 若玩家有輸入轉向方向：優先依據玩家指令轉向。
  - 若玩家無輸入或輸入阻擋方向：藍車依**順時針優先原則**自動尋找出路（優先嘗試車頭順時針 +90° 右轉，次選逆時針 -90° 左轉），全程不減速、不撞牆停滯。
- **AC5 (Dead End Auto 180° U-Turn)**：若藍車駛入三面皆牆之死胡同（Cul-de-sac），前方與兩側皆無通路時，順時針尋路邏輯自動遞補至反向，碰觸盡頭牆壁瞬間**自動執行 180 度原地大調頭**，立即反向衝出死胡同，全程不停車。
