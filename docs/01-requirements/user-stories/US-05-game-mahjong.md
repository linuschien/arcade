...
- **AC5 (Same-Tile Immediate Discard Lockout & Safe Follow Discard)**：
  - 當牌家當巡親自打出牌 $X$ 後，在下次輪到自己摸牌前，若其他牌家立刻跟打同張牌 $X$，**該出牌牌家禁止宣告榮和（胡牌）**。
...
- **AC3 (Strict Pass Lockout & Two Unlock Paths)**：
  - 當別家打出聽牌張而牌家選擇「過水 (Pass)」不胡時，立即進入過水鎖定狀態。
  - **合法解除途徑一（實質轉聽）**：牌家輪到自己摸牌後換張打牌，使手牌**實質轉聽全新牌張**，原聽牌張之過水鎖定立即解除（轉聽打出之牌受「同巡現打現抓禁令」約束，參見 US-05-03 AC5）。
