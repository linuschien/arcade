...
## 背景 (Background)
台灣16張麻將是 Arcade Stadium 平台上的第四個經典棋牌益智子遊戲。本模組規範 144 張牌庫結構、起手配牌與補花流程、16 張「5 面子 + 1 眼」胡牌型態驗證、動作優先權仲裁（吃碰槓胡）、四種槓牌與搶槓、500底/200台經典台數結算、聽牌與過水限制、海底 16 張鐵牌流局、向聽數牌效推進與危險度棄胡防守 AI 引擎、搬風抓位、擲骰開門、四圈一將流轉、連莊下莊狀態機以及圈風門風判定。
...
## US-05-02：16 張胡牌型態驗證（5 面子 + 1 眼）(Winning Hand Validation)

**身份**： 規則判定引擎 (Rule Engine)

> **As a** 規則判定引擎，  
> **I want to** 在玩家摸牌或宣告胡牌時驗證是否滿足「5 面子 + 1 對子（眼）」，  
> **So that** 精準判定是否達成 17 張牌（含胡牌張）的合法胡牌條件。

### 驗收條件 (Acceptance Criteria)
- **AC1 (Standard Hand 5 Melds + 1 Pair)**：判定玩家手牌與門前副露（吃、碰、槓）組合，總計須構成 5 組順子/刻子/槓子與 1 組對子（總計 17 張牌）。
- **AC2 (Special Hand Eight Pairs)**：支援特殊牌型「嚦咕嚦咕（八對半）」（8 組對子 + 1 張單張，且手牌必須全門清無任何副露）。
...
## US-05-05：台灣麻將 500底/200台 經典台數結算 (Taiwanese Fan Calculation)

**身份**： 計分系統 (Scoring System)

> **As a** 計分系統，  
> **I want to** 在達成胡牌時依據牌型、門前狀態與局況精準計算總台數並按 500底/200台 結算，  
> **So that** 精準結算各家輸贏點數與籌碼。
...
- **AC3 (Hand Pattern Fans)**：
  - 平胡（2台）、門清（1台）、自摸（1台）、門清自摸（3台）。
  - 碰碰胡（4台）、混一色（4台）、清一色（8台）、字一色（16台）。
  - 大三元（8台）、小三元（4台）、大四喜（16台）、小四喜（8台）、嚦咕嚦咕（8台）。
- **AC4 (Situational & Flower Fans)**：
  - 圈風台（1台）、門風台（1台）、三元牌刻子（各1台）、正花（1台/張）、花槓（2台）。
  - 槓上開花（1台）、海底撈月（1台）、搶槓（1台）、天胡（16台）、地胡（8台）、人胡（8台）。

---

## US-05-06：聽牌判定與過水/摸牌解除約束 (Winning Tile Calculation & Pass Lockout)

**身份**： 規則監督器 (Rule Supervisor)

> **As a** 規則監督器，  
> **I want to** 即時計算玩家聽牌清單並強制執行「過水」規則，  
> **So that** 防止玩家在同一巡內惡意挑選放銃對象。

### 驗收條件 (Acceptance Criteria)
- **AC1 (Winning Tile Tracking)**：系統即時演算手牌缺一張即胡的「聽牌張清單（Winning Tiles）」與剩餘張數。
- **AC2 (Pass Lockout Trigger)**：當別家打出玩家可胡之牌而玩家未宣告胡牌（過水）時，鎖定該玩家的榮和權利。
- **AC3 (Unlock on Draw & Discard)**：在玩家重新自摸一張牌並打出非胡之牌前，該巡內其他家打出的任何聽牌張皆禁止宣告胡牌。
...
### 驗收條件 (Acceptance Criteria)
- **AC1 (Shanten Evaluation)**：AI 遍歷當前 17 張手牌（摸牌後），計算打出任一張牌後的向聽數 $S$（距離聽牌差幾步）。
...
## US-05-13：四圈（東、南、西、北風圈）流轉與打完一將判定 (4-Wind Match Flow)

**身份**： 牌局排程器 (Match Scheduler)

> **As a** 牌局排程器，  
> **I want to** 追蹤「圈風（Round Wind）」與「門風（Seat Wind）」之狀態機切換，  
> **So that** 正確推進完整的一將（四圈）並判定遊戲結束。

### 驗收條件 (Acceptance Criteria)
- **AC1 (4-Wind Flow Progression)**：一將固定由 **「東風圈 $\rightarrow$ 南風圈 $\rightarrow$ 西風圈 $\rightarrow$ 北風圈」** 依序進行。
- **AC2 (Round Rotation)**：每圈內由東風位玩家開始當莊（「東風東」），莊家下莊後輪由下家當莊（南家 $\to$ 西家 $\to$ 北家）。
- **AC3 (Wind Advance on North Seat Down)**：當四位玩家皆當過一次莊（北家下莊）且無連莊時，該圈結束，進入下一風圈。
- **AC4 (Match Over & Summary)**：北風圈的北家下莊時，一將正式結束，觸發結算並透過 `ArcadeBridge.emit('GAME_OVER', summary)` 拋出四家總成績。
...
### 驗收條件 (Acceptance Criteria)
- **AC1 (Round Wind Fan)**：若手牌或副露中擁有與當前「圈風」相符的風牌刻子/槓子（如東風圈拿到東風刻子），結算加 1 台（圈風台）。
- **AC2 (Seat Wind Fan)**：若手牌或副露中擁有與自己當前「門風」相符的風牌刻子/槓子（如坐在南風位拿到南風刻子），結算加 1 台（門風台）。
