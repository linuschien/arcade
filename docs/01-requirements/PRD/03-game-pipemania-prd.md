...
|---|---|---|---|
| **起噴預備時間 $T_{\text{delay}}$** | $T_{\text{delay}}(L) = \max(10.0, 30.0 - 0.57 \times (L - 1))$ | **$30.00\text{s} \sim 10.00\text{s}$** | 秒 (開局至液體流動時間，保留終點拓撲規劃期) |
| **每格流速推進間隔 $T_{\text{flow}}$** | $T_{\text{flow}}(L) = \max(1000, \text{round}(3000 - 57.14 \times (L - 1)))$ | **$3000\text{ms} \sim 1000\text{ms}$** | 毫秒/格 (開局 3.0s，高難度 1.0s) |
| **水庫管推進耗時 $T_{\text{reservoir}}$** | $T_{\text{reservoir}}(L) = T_{\text{flow}}(L) \times 6.0$ | **$18000\text{ms} \sim 6000\text{ms}$** | 毫秒/格 (6倍時間，16.7%流速，戰略救場緩衝) |
...
| **起終點曼哈頓距離 $D$** | $D_{\text{manhattan}}(L) = \max(2, \lfloor 8 - 0.171 \times (L - 1) \rfloor)$ | **$8 \sim 2$ 格** | 開局 8 格 (留足 S 型繞路空間)，漸進至 2 格 (緊密對峙) |
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **L1** | 30.00s | 3000ms | 18000ms | 10 格 | 0 顆 | 0 根 | 8 格 | 正對起點 | **0.0%** | **15.0%** | **85.0%** (各12.1%) |
| **L5** | 27.71s | 2771ms | 16626ms | 12 格 | 0 顆 | 0 根 | 7 格 | 正對起點 | **0.0%** | **13.8%** | **86.2%** |
| **L8** | 26.00s | 2600ms | 15600ms | 14 格 | 1 顆 | 0 根 | 6 格 | 正對起點 | **0.0%** | **12.9%** | **87.1%** |
| **L9** | 25.43s | 2543ms | 15258ms | 14 格 | 1 顆 | 1 根 | 6 格 | 垂直正交 | **0.7%** | **12.6%** | **86.7%** |
| **L15** | 22.00s | 2200ms | 13200ms | 18 格 | 2 顆 | 2 根 | 5 格 | 垂直正交 | **5.0%** | **10.8%** | **84.2%** |
| **L20** | 19.14s | 1914ms | 11484ms | 21 格 | 3 顆 | 3 根 | 4 格 | 垂直正交 | **8.6%** | **9.3%** | **82.1%** |
| **L21** | 18.57s | 1857ms | 11142ms | 21 格 | 3 顆 | 3 根 | 4 格 | 背向起點 | **9.4%** | **9.0%** | **81.6%** |
| **L28** | 14.57s | 1457ms | 8742ms | 25 格 | 4 顆 | 4 根 | 3 格 | 背向起點 | **14.4%** | **6.9%** | **78.7%** |
| **L36** | 10.00s | 1000ms | 6000ms | 30 格 | 6 顆 | 6 根 | 2 格 | 背向起點 | **20.0%** | **5.0%** | **75.0%** (各10.7%) |
...
|---|---|---|---|---|---|
| **第 1 輪 (主線)** ($R=0$) | $L = 1 \sim 36$ | **30.00s ~ 10.00s** | **3000ms ~ 1000ms** | **18000ms ~ 6000ms** | 針對指定終點制打造的 36 關主線 |
| **第 2 輪 (Loop 1)** ($R=1$) | $L = 37 \sim 72$ | **27.00s ~ 9.00s** | **2700ms ~ 900ms** | **16200ms ~ 5400ms** | 起速重置為 2700ms (Wave Reset 喘息)，終速 900ms |
| **第 3 輪 (Loop 2)** ($R=2$) | $L = 73 \sim 108$ | **24.30s ~ 8.10s** | **2430ms ~ 810ms** | **14580ms ~ 4860ms** | 起速 2430ms，終速 810ms |
| **第 4 輪 (Loop 3)** ($R=3$) | $L = 109 \sim 144$ | **21.87s ~ 7.29s** | **2187ms ~ 729ms** | **13122ms ~ 4374ms** | 起速 2187ms，終速 729ms |
| **第 5 輪+ (極限)** ($R \ge 4$) | $L \ge 145$ | **5.00s** (保底極限) | **$T_{\text{flow}} \to 600\text{ms}$** (保底極限) | **3600ms** (保底極限) | 達到極限保護牆 |
