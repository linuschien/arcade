...
- **障礙與預設元件**：
  ...
  - **預設固定水管 (Preset Pipes)**：盤面上以螺栓固定、不可覆蓋的金色金屬水管。**預設水管之種類生成機率與發牌佇列完全一致，共享同套 Level 加權發牌算式 (Weighted Drop Rate)**。流經獲得 **「該水管基礎得分 + 200 點 Bonus」**（例：直管 $250$ 分，十字管 $300$ 分/軸，水庫管 $500$ 分）。
...
### 2.4 全域統一加權發牌機制 (Unified Weighted Drop Rate by Level L)
**此加權發牌規則由「佇列生成器」與「盤面預放水管生成器」100% 共享共用**：
- **$L < 4$**：水庫管發牌率 $0\%$、單向管發牌率 $0\%$。
- **$4 \le L < 7$**：水庫管發牌率 $P_{\text{reservoir}} = \max(5\%, 15\% - 1\% \times (L - 4))$，單向管 $0\%$。
- **$L \ge 7$**：
  - 單向管發牌率：$P_{\text{oneway}} = \min(20\%, 5\% + 1\% \times (L - 7))$
...
|---|---|---|---|
| **新手期 ($L = 1 \sim 8$)** | $D \ge 9$（遠距對角） | 終點開口**正對**起點方向 | 距離遠、路線直觀，容易順向對接 |
