...
|---|---|---|---|
| **起噴預備時間 $T_{\text{delay}}$** | $T_{\text{delay}}(L) = \max(1.0, 10.0 - 0.25 \times (L - 1))$ | **$10.00\text{s} \sim 1.25\text{s}$** | 秒 (開局至液體流動時間) |
| **每格流速推進間隔 $T_{\text{flow}}$** | $T_{\text{flow}}(L) = \max(500, 1500 - 28 \times (L - 1))$ | **$1500\text{ms} \sim 520\text{ms}$** | 毫秒/格 (下限封頂 500ms，2格/秒) |
| **水庫管推進耗時 $T_{\text{reservoir}}$** | $T_{\text{reservoir}}(L) = T_{\text{flow}}(L) \times 4.0$ | **$6000\text{ms} \sim 2080\text{ms}$** | 毫秒/格 (4倍時間，25%流速) |
| **通關目標長度 $N_{\text{target}}$** | $N_{\text{target}}(L) = \min(40, \lfloor 10 + 0.85 \times (L - 1) \rfloor)$ | **$10 \sim 40$ 格** | 進入終點前最少須連通格數 |
| **地圖障礙石頭數 $N_{\text{obstacle}}$** | $N_{\text{obstacle}}(L) = \min(12, \lfloor 0.35 \times (L - 1) \rfloor)$ | **$0 \sim 12$ 顆** | 盤面隨機障礙石頭數量 |
| **預設固定水管數 $N_{\text{fixed}}$** | $N_{\text{fixed}}(L) = \min(6, \max(0, \lfloor 0.18 \times (L - 3) \rfloor))$ | **$0 \sim 5$ 根** | $L=1\sim 8$為0，$L\ge 9$自然遞增 |
| **起終點曼哈頓距離 $D$** | $D_{\text{manhattan}}(L) = \max(2, \lfloor 12 - 0.28 \times (L - 1) \rfloor)$ | **$12 \sim 2$ 格** | 由遠距對角 (12格) 縮至相鄰 (2格) |
...
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **L1** | 10.00s | 1500ms | 6000ms | 10 格 | 0 顆 | 0 根 | 12 格 | 正對起點 | **0.0%** | **15.0%** | **85.0%** (各12.1%) |
| **L5** | 9.00s | 1388ms | 5552ms | 13 格 | 1 顆 | 0 根 | 10 格 | 正對起點 | **0.0%** | **13.8%** | **86.2%** |
| **L8** | 8.25s | 1304ms | 5216ms | 15 格 | 2 顆 | 0 根 | 10 格 | 正對起點 | **0.0%** | **12.9%** | **87.1%** |
| **L9** | 8.00s | 1276ms | 5104ms | 16 格 | 2 顆 | 1 根 | 9 格 | 垂直正交 | **0.7%** | **12.6%** | **86.7%** |
| **L15** | 6.50s | 1108ms | 4432ms | 21 格 | 4 顆 | 2 根 | 8 格 | 垂直正交 | **5.0%** | **10.8%** | **84.2%** |
| **L20** | 5.25s | 968ms | 3872ms | 26 格 | 6 顆 | 3 根 | 6 格 | 垂直正交 | **8.6%** | **9.3%** | **82.1%** |
| **L21** | 5.00s | 940ms | 3760ms | 27 格 | 7 顆 | 3 根 | 6 格 | 背向起點 | **9.4%** | **9.0%** | **81.6%** |
| **L28** | 3.25s | 744ms | 2976ms | 32 格 | 9 顆 | 4 根 | 4 格 | 背向起點 | **14.4%** | **6.9%** | **78.7%** |
| **L36** | 1.25s | 520ms | 2080ms | 40 格 | 12 顆 | 5 根 | 2 格 | 背向起點 | **20.0%** | **5.0%** | **75.0%** (各10.7%) |
...
|---|---|---|---|---|---|
| **第 1 輪 (主線)** ($R=0$) | $L = 1 \sim 36$ | **10.00s ~ 1.25s** | **1500ms ~ 520ms** | **6000ms ~ 2080ms** | 經典 36 關主線 |
...
| **第 4 輪 (Loop 3)** ($R=3$) | $L = 109 \sim 144$ | **7.29s ~ 1.00s** (保底 1s) | **1093ms ~ 379ms** | **4372ms ~ 1516ms** | 起速 1093ms，終速 379ms |
| **第 5 輪+ (極限)** ($R \ge 4$) | $L \ge 145$ | **1.00s** (保底極限) | **$T_{\text{flow}} \to 350\text{ms}$** (保底極限) | **1400ms** (保底極限) | 達到人體物理極限保護牆 |
