...
import { createRallyXGame, RallyXGame } from '../index';
...
vi.mock('phaser', () => {
  return {
    default: {
      ...
      Scene: class Scene {
        make = { graphics: vi.fn().mockReturnValue({ fillStyle: vi.fn(), fillCircle: vi.fn(), fillRect: vi.fn(), generateTexture: vi.fn(), destroy: vi.fn() }) };
...
describe('RallyXGame Entry Point Unit Tests', () => {
  let container: HTMLDivElement;
  let gameInstance: RallyXGame;

  beforeEach(() => {
    container = document.createElement('div');
    container.id = 'game-container';
    document.body.appendChild(container);
    gameInstance = createRallyXGame(container) as RallyXGame;
  });
  ...
  it('should instantiate RallyXGame successfully', () => {
    expect(gameInstance).toBeDefined();
  });

  it('should handle onPause and onResume cleanly', () => {
